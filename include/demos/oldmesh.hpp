...
public:
  ...
  TriangleSurface() : textures(nullptr), texture_nb(0), vao(0) {}
  void destroy() {
    if (textures != nullptr)
      delete[] textures;
    vao = 0;
  ...
  TriangleSurface(const Vertex &v1, const Vertex &v2, const Vertex &v3,
                  Texture *txts, unsigned int tnb)
      : p1(v1), p2(v2), p3(v3), textures(txts), texture_nb(tnb) {
    setup();
  }
  TriangleSurface(Vertex vs[element_nb], Texture *txts, unsigned int tnb)
      : p1(vs[0]), p2(vs[1]), p3(vs[2]), textures(txts), texture_nb(tnb) {
    setup();
  }
...
private:
  ...
  void setup() {
    ...
    //
    glBufferData(GL_ARRAY_BUFFER, vertex_nb * sizeof(Vertex), &vertices[0],
                 GL_STATIC_DRAW);

    // bind indices
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, index_nb * sizeof(unsigned int),
...
public:
  ...
  void draw(Shader shdr) {
    ...
    glBindVertexArray(vao);
    glDrawElements(GL_TRIANGLES, index_nb, GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
    //
...
public:
  ...
  TriangleSurface *ts;
  Mesh1() : nb_triangles(0), ts(nullptr) {}
  //~Mesh() { delete[] ts; }

  Mesh1(TriangleSurface *triangles, unsigned int nbt) : nb_triangles(nbt) {
...
public:
  ...
  SimpleShape() : nb_shape(0), in_tangent(false), stype(ShapeChoice::CUBE) {}
  ...
  void draw() {
    ...
    switch (stype) {
    case ShapeChoice::CUBE: {
      drawfn = in_tangent ? renderCubeInTangentSpaceS : renderCubeS;
      break;
    }
    case ShapeChoice::CUBE_D: {
  ...
  void draw_line(const std::vector<std::array<glm::vec3, 2>> &verts) {
...
public:
  ...
  SimpleShape shape = SimpleShape(1, false, ShapeChoice::CUBE);
  AmmoRound() {}
  AmmoRound(ShotType st, const glm::vec3 &s = glm::vec3(1.0f))
      : stype(st), scale(s) {}
...
TriangleSurface mk_triangle_surface(const glm::vec3 ps[3],
                                    const glm::vec2 ts[3], const glm::vec3 &n) {
  //
  Vertex *vs = new Vertex[TriangleSurface::element_nb];
  mk_triangle_vertices(ps, ts, n, vs);
  return TriangleSurface(vs, nullptr, 0);
};
TriangleSurface mk_triangle_surface(const glm::vec3 &p1, const glm::vec3 &p2,
                                    const glm::vec3 &p3, const glm::vec2 &t1,
                                    const glm::vec2 &t2, const glm::vec2 &t3,
                                    const glm::vec3 &n) {
...
TriangleSurface mk_triangle_surface(float vs[15], float n[3]) {
...
Mesh1 mk_triangle(const glm::vec3 ps[3], const glm::vec2 ts[3],
                  const glm::vec3 &n) {
...
Mesh1 mk_cube() {
  ...
  TriangleSurface tris[] = {tri1, tri2, tri3, tri4,  tri5,  tri6,
                            tri7, tri8, tri9, tri10, tri11, tri12};
  return Mesh1(tris, 12);
};
...
Mesh1 mk_plane() {
  ...
  glm::vec3 stri_pos[3] = {glm::vec3(25.0f, -0.5f, 25.0f),
                           glm::vec3(-25.0f, -0.5f, -25.0f),
                           glm::vec3(25.0f, -0.5f, -25.0f)};
  glm::vec2 stri_tex[3] = {glm::vec2(25.0f, 0.0f), glm::vec2(0.0f, 25.0f),
                           glm::vec2(25.0f, 25.0f)};
  glm::vec3 s_norm = glm::vec3(0.0f, 1.0f, 0.0f);

  auto tri1 = mk_triangle_surface(ftri_pos, ftri_tex, f_norm);
  auto tri2 = mk_triangle_surface(stri_pos, stri_tex, s_norm);

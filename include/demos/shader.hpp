...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, Shader s);
};
std::ostream &operator<<(std::ostream &out, Shader s) {
  out << s.shaderName << std::endl;
  return out;
}
...
Shader::Shader(std::string *shaderPaths, std::string *shaderTypes,
               unsigned int nb_paths) {
  programId = glCreateProgram();
  GLuint *shaderIds = new GLuint[nb_paths];
  for (unsigned int i = 0; i < nb_paths; i++) {
    //
    GLuint shdrId = loadShader(shaderPaths[i].c_str(), shaderTypes[i].c_str());
...
Shader mk_const_color_mesh_shader() {
  const GLchar *vertex_path = "media/demos/glsl/meshcolor.vert";
...
Shader mk_pointlight_lamp_shader() {
  ...
  const GLchar *fragment_source = "media/demos/glsl/basic_color_light.frag";
  return Shader(vertex_path, fragment_source);
}
Shader mk_depth_shader() {
  const GLchar *vertex_path = "media/demos/glsl/shadow_depth.vert";
  const GLchar *fragment_source = "media/demos/glsl/shadow_depth.frag";
  return Shader(vertex_path, fragment_source);
}
